...
) = this.getPack(phaseName.split(".").first()).add(Transform(phaseName, object : SceneTransformer() {
    override fun internalTransform(phaseName: String, options: Map<String, String>) = body(phaseName, options)
}))

fun SootClass.includeSuperClasses() = generateSequence({ this }, {
    if (it.hasSuperclass()) it.superclass else null
}).filter { it != Scene.v().objectType.sootClass }.toList()


private val testMethodAnnotations = setOf(
    "Lorg/junit/Test;",
    "Lorg/junit/jupiter/api/Test;"
)

private val setupAnnotations = setOf(
    "Lorg/junit/Before;",
    "Lorg/junit/After;",
...
fun SootMethod.isTestMethod(): Boolean = this.getTag("VisibilityAnnotationTag")
    .let { it as? VisibilityAnnotationTag }?.annotations?.any { it.type in testMethodAnnotations } ?: false

fun SootClass.isFunctionalInterface() = this.getTag("VisibilityAnnotationTag")
    .let { it as? VisibilityAnnotationTag }?.annotations?.any { it.type == "Ljava/lang/FunctionalInterface;" }
    ?: false
...
fun generateMethodCallLevels(
    ...
    return generateSequence({ seed.toSet() }, { ms ->
        ms.flatMap { callGraph.edgesOutOf(it).asSequence().toList() }
            .map { it.tgt.method() }
...
fun List<SootClass>.includeInterfaces() = if (this.isEmpty()) emptyList() else Sequence {
...
fun configureSoot(project: Project, java8RuntimePath: File) {
    with(Options.v()) {
        set_output_format(Options.output_format_none)
        set_src_prec(Options.src_prec_class)

        val sootCp = (
                setOf(project.targetClassDir, project.testTargetClassDir).filter { it.exists() }
                    .map { it.absolutePath }
