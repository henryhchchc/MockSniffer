...
class TestInstrumentCommand : CliktCommand(name = "test-bytecode-instrument") {
    ...
    private fun instrumentByteCode(repo: CodeRepository) {
        ...
        val copyBackPairs = repo.projects
                .parallelStream()
                .flatMap {
                    semaphore.acquire()
    ...
    private fun startSootProcess(project: Project, rtJarPath: String): List<Pair<String, String>> {
        val tempDir = Files.createTempDirectory("soot_temp").toAbsolutePath().normalize().toString()
        ProcessBuilder(
            javaExecutable.absolutePath, "-jar", jarName,
            PreProjectInstrumentCommand.commandName,
            "-r", project.codeRepository.directory.absolutePath,
        ...
        ProcessBuilder(
            ...
            .waitFor()
        return listOf(
            Path.of(tempDir, "src").toAbsolutePath().normalize().toString() to project.targetClassDir.absolutePath,
            Path.of(tempDir, "test").toAbsolutePath().normalize().toString() to project.testTargetClassDir.absolutePath
        )
    }
}
