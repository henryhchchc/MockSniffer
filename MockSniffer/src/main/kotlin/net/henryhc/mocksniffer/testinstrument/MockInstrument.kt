...
class MockInstrument : BodyTransformer() {
    ...
    private fun instrumentParams(stmt: Stmt, units: UnitPatchingChain, body: Body) {
        ...
        if (stmt.invokeExpr is InstanceInvokeExpr ||
                (stmt.invokeExpr is SpecialInvokeExpr && stmt.invokeExpr.methodRef.tryResolve()?.isConstructor == true)) {
            for (i in 0 until stmt.invokeExpr.argCount) {
                ...
                val invStmt = Jimple.v().newStaticInvokeExpr(recordObjMethod.makeRef(),
                                alias,
                                IntConstant.v(i),
