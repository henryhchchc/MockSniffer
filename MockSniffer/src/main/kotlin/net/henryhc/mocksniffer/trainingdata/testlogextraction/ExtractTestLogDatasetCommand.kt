...
class ExtractTestLogDatasetCommand : CliktCommand(name = "test-log-dataset") {
    ...
    private fun constructDataset(
        ...
        val objParams = CSVFormat.DEFAULT
    ...
    private val depBlacklist = setOf(
            "java.lang.Object",
            "java.lang.String",
    ...
    private fun createDataset(
        ...
        val entries = objParams.map { Pair(it, "param") }
                .chunkedByProcessorCount().parallelStream()
                .flatMap { pg ->
    ...
    private fun finalObjType(entry: TestLogParamEntry) = when {
        entry.objType != "real" -> entry.objType
        mockRegexps.any { entry.typeName.contains(it) } -> "mock"
        else -> "real"
    }

    private fun constructCUTMap(sourceRepo: CodeRepository, testClassNames: Set<String>) =
            testClassNames.associateWith { findCUT(sourceRepo, it) }
                    .filterValues { it != null }.mapValues { (_, v) -> v as String }

    private fun findCUT(sourceRepo: CodeRepository, testClassName: String): String? {
        val cut = testClassName
                .trimWord("test")
                .trimWord("tests")
                .trimWord("testcase")
        when {
    ...
    private fun String.trimWord(word: String, trimStart: Boolean = true, trimEnd: Boolean = true): String {
