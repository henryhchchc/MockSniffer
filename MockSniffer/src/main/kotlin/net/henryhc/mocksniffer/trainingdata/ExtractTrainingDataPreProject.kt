...
class ExtractTrainingDataPreProject : PreProjectFeatureExtractor("extract-training-data-pre-project") {
    ...
    override fun run() {
        ...
        val inputData = inputCsv.bufferedReader().use { reader ->
            CSVFormat.DEFAULT.withHeader(*datasetEntryCsvHeader).parse(reader)
                .drop(1)
                .map { it.toDatasetEntry() }.filter { it.classUnderTest.isNotEmpty() }
        }
        ...
        PackManager.v().addSceneTransformer("wstp.extractfeatures") { _, _ ->
            ...
            val testSetupEntries = inputData.parallelStream()
                .filter {
                    val testClass = Scene.v().getSootClass(it.testClassName)
                    testClass.methods.any { m -> m.name == it.testMethodName && m.isTestSetupMethod() }
                }.flatMap { entry ->
                    Scene.v().getSootClass(entry.testClassName).methods.filter { it.isTestMethod() }.map {
                        entry.copy(testMethodName = it.name)
                    }.stream()
                }.collect(Collectors.toList())

            val resultDataset =
                (testSetupEntries + inputData).parallelStream().filter { it.src == "param" }.map { entry ->
                    semaphore.acquire()
    ...
    private fun extractFeatures(entry: DatasetEntry, classNames: Set<String>, project: Project): TrainingDataEntry? {
        ...
        val testMethod = testClass.methods.filter { it.isTestMethod() }
            .firstOrNull { it.name == entry.testMethodName } ?: return null
        val involvedTestMethods = testMethod.expandCallGraph {
        ...
        val methodsUnderTest = generateMethodCallLevels(involvedTestMethods)
            .take(cgExpand)
            .flatten()
        ...
        val callSiteOnDep = methodsUnderTest
...
//            it.invocationsDependsOnBranch
        ...
        val dominance = methodsUnderTest
            .filter { it.isConcrete }
